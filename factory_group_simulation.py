...
class Mic_factory:
    def __init__(self, env):
        self.material = simpy.Container(env, capacity = material_capacity, init = 6000)
...
def Uassembly(env, mic_factory):
    while True:
        yield mic_factory.material.get(1) 
        yield mic_factory.material_treatment.get(1) 
        yield mic_factory.parts_processing.get(1) 
        uassembly_time = random.gauss(mean_uassembly, std_uassembly)
        yield env.timeout(uassembly_time)
        yield mic_factory.underframe_assembly.put(1)
...
def Rassembly(env, mic_factory):
    while True:
        ...
        yield mic_factory.roof_assembly.put(1)

def Tassembly(env, mic_factory):
    while True:
        yield mic_factory.material_treatment.get(1) 
        yield mic_factory.parts_processing.get(1) 
        yield mic_factory.underframe_assembly.get(1) 
        yield mic_factory.roof_assembly.get(1) 
        rassembly_time = random.gauss(mean_tassembly, std_tassembly)
        yield env.timeout(rassembly_time)
        yield mic_factory.total_assembly.put(1)

def Welding(env, mic_factory):
    while True:
        yield mic_factory.total_assembly.get(1) 
        treatment_time = random.gauss(mean_welding, std_welding)
        yield env.timeout(treatment_time)
        yield mic_factory.welding.put(1)

def Painting(env, mic_factory):
    while True:
        yield mic_factory.welding.get(1) 
        treatment_time = random.gauss(mean_painting, std_painting)
        yield env.timeout(treatment_time)
        yield mic_factory.painting.put(1)

def Renovation(env, mic_factory):
    while True:
        yield mic_factory.painting.get(1) 
        treatment_time = random.gauss(mean_renovate, std_renovate)
        yield env.timeout(treatment_time)
        yield mic_factory.renovation.put(1)
